...
test.describe('Dashboard UX browser acceptance', () => {
  ...
  test('E1-S3-AC1', async ({ page }) => { await load(page); const name = page.locator('.session-name'); await expect(name).toHaveCSS('white-space', 'nowrap'); await expect(name).toHaveCSS('text-overflow', 'ellipsis'); expect(await name.evaluate(el => el.clientHeight <= parseFloat(getComputedStyle(el).lineHeight) + 1)).toBe(true); });
  ...
  test('E3-S1-AC3', async ({ page }) => { await load(page); await expect(page.locator('.turn-assistant .markdown-body h2')).toHaveText('Result'); await expect(page.locator('.turn-assistant .markdown-body ul')).toBeVisible(); await expect(page.locator('.turn-user .markdown-body code')).toBeVisible(); });
  ...
  test('E3-S2-AC1', async ({ page }) => { await load(page); await expect(page.locator('.turn-assistant')).toHaveCSS('border-left-width', '3.2px'); });
  test('E3-S2-AC2', async ({ page }) => { await load(page); const assistant = page.locator('.turn-assistant'); await expect(assistant).toHaveCSS('border-left-width', '3.2px'); expect(await assistant.evaluate(el => el.style.getPropertyValue('--lane-color'))).toBeTruthy(); });
  ...
  test('E3-S2-AC4', async ({ page }) => { await load(page); await expect(page.locator('.turn-dot')).toHaveCount(4); await expect(page.locator('.turn-dot').first()).toHaveCSS('background-color', 'rgb(59, 130, 246)'); });
