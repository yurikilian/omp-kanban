...
const timeline = {
  ...
  root: {
    ...
    events: [
      ...
      { role: 'tool_execution', agent: 'main', lane: 0, ts: '2026-07-25T12:03:00.000Z', toolName: 'bash', content: 'tool output', resultContent: 'tool output', children: [{ agent: 'worker', lane: 1, count: 1, firstTs: '2026-07-25T12:03:10.000Z', lastTs: '2026-07-25T12:03:10.000Z', events: [{ role: 'system', agent: 'worker', lane: 1, ts: '2026-07-25T12:03:10.000Z', content: 'Nested system event' }] }] },
    ],
  },
};
...
test.describe('Dashboard UX browser acceptance', () => {
  ...
  test('E2-S1-AC1', async ({ page }) => { await load(page); const scrollables = await page.locator('.session-detail *').evaluateAll(elements => elements.filter(el => ['auto', 'scroll'].includes(getComputedStyle(el).overflowY)).map(el => el.matches('.session-detail-messages') ? '.session-detail-messages' : el.matches('.markdown-body pre') ? '.markdown-body pre' : `${el.tagName.toLowerCase()}.${el.className}`)); expect(scrollables).toEqual(['.session-detail-messages']); });
  ...
  test('E3-S2-AC1', async ({ page }) => { await load(page); const width = await page.locator('.timeline-root').evaluate(el => parseFloat(getComputedStyle(el).borderLeftWidth)); expect(width).toBeGreaterThan(0); expect(width).toBeLessThanOrEqual(4); });
  test('E3-S2-AC2', async ({ page }) => { await load(page); const root = page.locator('.timeline-root'); const width = await root.evaluate(el => parseFloat(getComputedStyle(el).borderLeftWidth)); expect(width).toBeGreaterThan(0); expect(width).toBeLessThanOrEqual(4); expect(await root.evaluate(el => el.style.getPropertyValue('--lane-color'))).toBeTruthy(); });
  ...
  test('E3-S2-C3-guide-continuity', async ({ page }) => { await load(page); await page.getByRole('button', { name: /worker/ }).click(); const guides = await page.locator('.turn-wrap').evaluateAll(wraps => wraps.map(wrap => { const dot = wrap.querySelector('.turn-dot') as HTMLElement | null; const section = wrap.closest('.timeline-root, .timeline-nested') as HTMLElement | null; const style = section ? getComputedStyle(section) : null; const rect = section?.getBoundingClientRect(); return { dotCenter: dot ? dot.getBoundingClientRect().x + dot.getBoundingClientRect().width / 2 : null, guideCenter: rect ? rect.x + parseFloat(style?.borderLeftWidth ?? '0') / 2 : null, guideWidth: style?.borderLeftWidth ?? '0px' }; })); expect(guides.every(guide => guide.guideWidth !== '0px' && Math.abs((guide.dotCenter ?? 0) - (guide.guideCenter ?? 0)) <= 1)).toBe(true); });
